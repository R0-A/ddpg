...
GUMBEL_DIST = Gumbel(loc=torch.tensor([0.0]), scale=torch.tensor([1.0]))
EXP_DIST = Exponential(rate=torch.tensor([1.0]))
...
class STGS(GradientEstimator):
    ...
    def __call__(self, logits, need_gradients=None):
        ...
        gumbel_noise = GUMBEL_DIST.sample(logits.shape).squeeze(-1).to(device)
        perturbed_logits = (logits + gumbel_noise) / self.temperature  # ~ Gumbel(logits,tau)
        y_soft = softmax(perturbed_logits, dim=-1)
        y_hard = one_hot(y_soft.argmax(dim=-1), num_classes=logits.shape[-1]).to(logits.dtype)
...
class GRMCK(GradientEstimator):
    ...
    def _conditional_gumbel(self, logits, DD):
        ...
        # iid. exponential
        device = logits.device
        EE = EXP_DIST.sample([self.kk, *logits.shape]).squeeze(-1).to(device)
    ...
    def __call__(self, logits, need_gradients=True):
        DD = OneHotCategorical(logits=logits).sample().to(logits.device, dtype=logits.dtype)
        if need_gradients:
